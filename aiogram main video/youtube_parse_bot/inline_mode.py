...
async def process_search_video (message:types.Message, state: FSMContext):
    ...
    for i in range(len(videos)):
        try:
            await bot.send_message(message.chat.id, videos[i].get_attribute('href'), reply_markup=keyboard)
        except:
            continue
...
async def search_channel(message:types.Message, state: FSMChannel):
    ...
    for card in range(len(videos_cards)):
        try:
            await bot.send_message(message.chat.id, videos_cards[card].get_attribute('href'))
        except:
            continue
